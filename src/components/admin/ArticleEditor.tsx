...
export default function ArticleEditor({ initial }: { initial?: ArticleFormValues }) {
  ...
  async function save(status?: 'draft' | 'published') {
    ...
    try {
      const res = await fetch(
        isNew ? '/api/admin/articles' : `/api/admin/articles/${initial!._id}`,
        {
      ...
      setValues((v) => ({ ...v, status: data.article.status, slug: data.article.slug }));
      setNotice(
  ...
  async function remove() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/admin/articles/${initial!._id}`, { method: 'DELETE' });
      if (!res.ok) {
        const data = await res.json();
